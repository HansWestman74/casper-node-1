...
use super::{
    faucet_test_helpers::{
        ...
        FaucetDeployHelper, FaucetInstallSessionRequestBuilder, FundAccountRequestBuilder,
    },
...
fn should_not_fund_once_exhausted() {
    ...
    // To use the faucet_configu_request we need to set the faucet contract_hash
...
fn should_allow_installer_to_fund_freely() {
    ...
    let faucet_contract_hash = get_faucet_contract_hash(&builder, installer_account);
    let _faucet_contract = get_faucet_contract(&builder, installer_account);
    let faucet_purse = get_faucet_purse(&builder, installer_account);
