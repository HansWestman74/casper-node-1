    ...
    match &event.data {
        ...
        &SseData::DeployAccepted {
            ...
            Some(Ok(WarpServerSentEvent::default()
                ...
                .id(event.id.expect("event should have id").to_string()))) //?expect
        }
    }
}
                ...
                match result {
                    Ok(BroadcastChannelMessage::ServerSentEvent(event)) => {
                        if let Some(id) = event.id {
                            if cloned_initial_ids.read().expect("cloned_initial_ids lock should not be poisoned")//?expect 
                                .contains(&id) {
        ...
        .map(move |event| {
            if let Some(id) = event.id {
                let _ = initial_stream_ids
                    .write()
                    .expect("initial_stream_ids lock should not be poisoned") //?expect
                    .insert(id);
            }
            Ok(event)
        })
