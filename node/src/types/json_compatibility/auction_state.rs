...
static ERA_VALIDATORS: Lazy<EraValidators> = Lazy::new(|| {
    ...
    let secret_key_1 = SecretKey::ed25519_from_bytes([42; SecretKey::ED25519_LENGTH]).unwrap(); //?unwrap
...
static BIDS: Lazy<Bids> = Lazy::new(|| {
    ...
    let validator_secret_key =
        SecretKey::ed25519_from_bytes([42; SecretKey::ED25519_LENGTH]).unwrap(); //?unwrap
    let validator_public_key = PublicKey::from(&validator_secret_key);
    // NOTE: Safe to unwrap since [u8] has correct length
    let delegator_secret_key =
        SecretKey::ed25519_from_bytes([43; SecretKey::ED25519_LENGTH]).unwrap(); //?unwrap
